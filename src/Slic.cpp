...
void Slic::findCenters(Mat& frame)
{
	...
	parallel_for(Range(0, m_allCenters.size()), [&](const Range& range)
	{
...
void Slic::updateCenters(Mat& frame)
{
	...
	parallel_for(Range(0, m_height), [&](const Range& range)
	{
		for (int i = range.start; i < range.end; i++)
		{
			int* m_labels_ptr = m_labels.ptr<int>(i);
			Vec3f* frame_ptr = frame.ptr<Vec3f>(i);
			for (int j = 0; j < m_width; j++)
			{

				int idxC = m_labels_ptr[j];
				if (idxC != -1) {
					Vec3f lab = frame_ptr[j];
...
void Slic::enforceConnectivity(Mat& frame)
{
	int label = 0, adjlabel = 0;
	int lims = (m_width * m_height) / (m_nSpx);
	lims = lims >> 2;
	if (lims < 2)return;
	Mat newLabels(m_height, m_width, CV_32SC1, Scalar(-1));
	int * newLabelsPtr = newLabels.ptr<int>();
		...
		for (int j = 0; j < m_width; j++)
		{
			...
			if (newLabelsPtr[i*m_width + j] == -1)
			{
				...
				for (int k = 0; k < 4; k++)
				{
					...
					{
						if (newLabelsPtr[y*m_width + x] >= 0)
						{
							adjlabel = newLabelsPtr[y*m_width + x];
						}
					}
				}
				int count = 1;
				for (int c = 0; c < count; c++)
				{
					for (int k = 0; k < 4; k++)
					{
						...
						if (x >= 0 && x < m_width && y >= 0 && y < m_height)
						{
							if (newLabelsPtr[y*m_width + x] == -1 && m_labels_ptr[j] == m_labels.at<int>(y, x))
							{
								elements.push_back(Point(x, y));
								newLabelsPtr[y*m_width + x] = label;//m_labels[i][j];
								count += 1;
							}
						}
					}
				...
				{
					for (int c = 0; c < count; c++) {
						newLabelsPtr[elements[c].y*m_width + elements[c].x] = adjlabel;
					}
					label -= 1;
				}
				label += 1;
